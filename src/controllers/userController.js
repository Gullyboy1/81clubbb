...
const verifyCode = async (req, res) => {
    ...
    if (user.time_otp - now <= 0) {
        request(`https://www.fast2sms.com/dev/bulkV2?authorization=UUesQqUATIKkkni5UbL569oUWyAru4RZo4Tmr8m3i6Qb86NjNrJDy2ZIfP08&variables_values=${otp}&route=otp&numbers=${user.phone}`, async (error, response, body) => {
...
const recharge = async (req, res) => {
    ...
    if (type != 'cancel' && type != 'submit' && type != 'submitauto') {
        if (!auth || !money || money <= 298) {
            return res.status(200).json({
                message: 'Minimum recharge 299',
                status: false,
                timeStamp: timeNow,
            })
        }
